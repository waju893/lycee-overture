...
function handleStartGame(state: GameState, action: Extract<GameAction, { type: 'START_GAME' }>): GameState {
  ...
  // Preserve replay/log/event history that was already recorded in reduceGameState
  // before this handler was entered. Without this, START_GAME disappears from
  // replayEvents and replay determinism breaks.
  ...
  switch (action.type) {
    ...
    case 'ADVANCE_PHASE':
      if (next.turn.phase === 'wakeup') {
        ...
        const drawCount =
          next.turn.turnNumber <= 1 && next.turn.firstPlayer === nextPlayer ? 1 : 2;
