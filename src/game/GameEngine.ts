...
function beginTurnAndEnterMain(state: GameState, playerId: PlayerID, incrementTurn: boolean): void {
  ...
  appendLog(state, `${playerId} ??Í∞úÏãú??);
  appendLog(state, '??Í∞úÏãú???†Î∞ú ?®Í≥º Ï≤òÎ¶¨ ?ÑÎ£å');
  state.turn.phase = 'main';
  state.turn.priorityPlayer = playerId;
  appendLog(state, 'Î©îÏù∏ ?òÏù¥Ï¶?);
}
...
function finalizeAttackResponses(state: GameState): void {
  ...
  appendLog(state, selectableDefenderExists ? 'Î∞∞Ì? Ï§??ÅÌÉú ÏßÑÏûÖ (Î∞©Ïñ¥ ?†ÌÉù Í∞Ä??' : 'Î∞∞Ì? Ï§??ÅÌÉú ÏßÑÏûÖ (Î∞©Ïñ¥??ÎØ∏Ï???');
}
...
function resolveCurrentBattle(state: GameState): void {
  ...
  if (defenderCardId) {
    ...
    if (found && found.playerId === defenderPlayerId) {
      ...
      appendLog(state, 'Î∞∞Ì? Ï¢ÖÎ£å (down = battle destroy)');
      clearBattleState(state);
  ...
  appendLog(state, `ÏßÅÏ†ë Í≥µÍ≤© Ï≤òÎ¶¨ (${dmg})`);
  clearBattleState(state);
  flushNormalizationAndTriggers(state, eventStartIndex);
}
...
function openAttackResponseWindow(state: GameState, action: Extract<GameAction, { type: 'DECLARE_ACTION' }>): void {
  ...
  appendLog(state, 'Í≥µÍ≤© ?†Ïñ∏');
  appendLog(state, 'Í≥µÍ≤© ?†Ïñ∏ ?Ä??Ï∞?ÏßÑÏûÖ');
}
  ...
  switch (action.type) {
    ...
    case 'ADVANCE_PHASE':
      if (next.turn.phase === 'wakeup') {
        ...
        appendLog(next, '??Ï¢ÖÎ£å???†Î∞ú ?®Í≥º Ï≤òÎ¶¨ ?ÑÎ£å');
    ...
    case 'DECLARE_ACTION': {
      ...
      if (action.kind === 'useCharacter') appendLog(next, '?±Ïû• ?†Ïñ∏');
      if (action.kind === 'useEvent') appendLog(next, '?¥Î≤§???¨Ïö© ?†Ïñ∏');
      if (action.kind === 'useArea') appendLog(next, '?êÎ¶¨??Î∞∞Ïπò ?†Ïñ∏');
      if (action.kind === 'useItem') appendLog(next, '?•ÎπÑ ?†Ïñ∏');
      if (action.kind === 'useAbility') appendLog(next, '?•Î†• ?¨Ïö© ?†Ïñ∏');
      if (action.kind === 'chargeCharacter') appendLog(next, 'Ï∞®Ï? ?†Ïñ∏');
      return next;
    }
    case 'PASS_PRIORITY':
    ...
    case 'SET_DEFENDER': {
      ...
      appendLog(next, selectedDefenderId ? 'Î∞∞Ì? Ï§??ÅÌÉú ÏßÑÏûÖ (Î∞©Ïñ¥??ÏßÄ??' : 'Î∞∞Ì? Ï§??ÅÌÉú ?†Ï? (Î∞©Ïñ¥ ????');
      return next;
    }
    default:
      return next;
