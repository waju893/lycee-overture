...
import { applyEffectDestroyToFieldCard, reduceGameState } from '../GameEngine';
...
describe('EffectDestroySemantics', () => {
  it('effect destroy emits destroy + left field, moves to discard, and does not emit down', () => {
    let state = buildReadyToMainState('P1');

    const victim = makeCharacter('VICTIM', 'P2', 'VICTIM', 2);
    state.players.P2.hand.unshift({ ...victim, location: 'hand' });
    ...
    const next = applyEffectDestroyToFieldCard(state, {
      affectedPlayerId: 'P2',
      cardId: 'VICTIM',
      sourcePlayerId: 'P1',
      sourceCardId: 'SOURCE_CARD',
      sourceEffectId: 'destroy_effect',
    });

    expect(next.players.P2.field.DF_LEFT.card).toBeNull();
    expect(next.players.P2.discard.some((card) => card.instanceId === 'VICTIM')).toBe(true);

    const downed = next.events.filter((event) => event.type === 'CARD_DOWNED' && event.cardId === 'VICTIM');
    const destroyed = next.events.filter((event) => event.type === 'CARD_DESTROYED' && event.cardId === 'VICTIM');
    const leftField = next.events.filter((event) => event.type === 'CARD_LEFT_FIELD' && event.cardId === 'VICTIM');

    expect(downed.length).toBe(0);
