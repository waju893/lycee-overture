...
export function isAF(slot: FieldSlot): boolean {
  return slot.startsWith("AF");
}

export function isDF(slot: FieldSlot): boolean {
  return slot.startsWith("DF");
}
...
export function createInitialGameState(params: {
  ...
  return {
    players: {
      P1: createInitialPlayerState("P1", params.p1Deck),
      P2: createInitialPlayerState("P2", params.p2Deck),
    },
    turn: {
      turnNumber: 1,
      activePlayer: "P1",
    ...
    battle: {
      isActive: false,
      attackDeclarationId: undefined,
      attackerPlayerId: undefined,
      defenderPlayerId: undefined,
      attackerCardId: undefined,
      defenderCardId: undefined,
      supportAttackers: [],
      supportDefenders: [],
      battleEndedByBothPass: false,
    },
    declarationStack: [],
    pendingTriggers: [],
    resolvingTriggerIds: [],
    ...
    winner: null,
    seed: params.seed,
    lastResolvedDeclarationId: undefined,
  };
}
...
export function findCardController(
  state: GameState,
  cardId: string,
): PlayerID | null {
...
export function canBuildDeck(cards: CardRef[]): RuleViolation[] {
  ...
  if (cards.length !== 60) {
    violations.push({
      ...
      message: `덱 장수는 정확히 60장이어야 합니다. 현재 ${cards.length}장입니다.`,
    });
  }

  const counts = new Map<string, number>();
  for (const card of cards) {
    const key = normalizeCardNo(card.cardNo);
  ...
  for (const [cardNo, count] of counts.entries()) {
    if (count > 4) {
      violations.push({
        code: "DECK_DUPLICATE_INVALID",
        message: `${cardNo}는 같은 번호 카드 4장 제한을 초과했습니다. 현재 ${count}장입니다.`,
      });
    }
  }
...
export function rollFirstPlayer(forcedFirstPlayer?: PlayerID): {
  ...
  if (forcedFirstPlayer) {
    return {
      firstPlayer: forcedFirstPlayer,
      secondPlayer: getOpponent(forcedFirstPlayer),
      rerollCount: 0,
      p1Roll: forcedFirstPlayer === "P1" ? 20 : 1,
      p2Roll: forcedFirstPlayer === "P2" ? 20 : 1,
    };
  }
...
export function extractLeaderToHand(
  state: GameState,
  playerId: PlayerID,
): CardRef | null {
...
export function performMulligan(
  ...
  if (!state.startup.active) {
    violations.push(
      fail("멀리건은 스타트업 단계에서만 가능합니다.", "TIMING_INVALID")[0],
    );
    return violations;
  }

  if (state.startup.mulliganUsed[playerId]) {
    violations.push(
      fail("멀리건은 1회만 가능합니다.", "MULLIGAN_ALREADY_USED")[0],
    );
    return violations;
  }
...
export function validateKeepOrMulligan(
  state: GameState,
  playerId: PlayerID,
): RuleViolation[] {
  const violations: RuleViolation[] = [];
  ...
  if (!state.startup.active) {
    violations.push(
      fail("스타트업이 끝난 뒤에는 선택할 수 없습니다.", "STARTUP_FINISHED")[0],
    );
  }

  if (state.startup.keepDecided[playerId]) {
    violations.push(
      fail("이미 시작 패 결정을 마쳤습니다.", "ALREADY_DECIDED")[0],
    );
  }
...
export function finalizeStartupState(state: GameState): void {
  ...
  for (const playerId of ["P1", "P2"] as const) {
    ...
    const leader = state.players[playerId].hand.find(
      (card) => card.instanceId === leaderCardId,
    );
    if (leader) {
      leader.revealed = false;
    }
  }
...
export function getWarmupDrawCount(state: GameState): number {
  ...
  const isFirstPlayerTurn = state.turn.activePlayer === state.startup.firstPlayer;

  if (isFirstTurn && isFirstPlayerTurn && !state.turn.firstPlayerDrawFixed) {
    return 1;
  }
...
export function findAttackableCharacters(
  state: GameState,
  playerId: PlayerID,
): string[] {
  const player = state.players[playerId];
  ...
  for (const slot of FIELD_SLOTS) {
    ...
    if (!card) continue;
    if (!isAF(slot)) continue;
    if (card.isTapped) continue;
    if (card.canAttack === false) continue;
...
export function findEligibleDefenders(
  state: GameState,
  ...
  for (const slot of FIELD_SLOTS) {
    const card = player.field[slot].card;
    if (!card) continue;
    if (!isDF(slot)) continue;
    if (getColumn(slot) !== column) continue;
    if (card.isTapped) continue;
    if (card.canBlock === false) continue;
    result.push(card.instanceId);
  }
...
export function validateCharacterEntry(
  state: GameState,
  playerId: PlayerID,
  card: CardRef,
  slot: FieldSlot,
  ...
  if (state.turn.activePlayer !== playerId || state.turn.phase !== "main") {
    violations.push(
      fail("캐릭터 등장은 자기 메인페이즈에만 가능합니다.", "TIMING_INVALID")[0],
    );
  }

  if (card.cardType !== "character") {
    violations.push(
      fail("캐릭터 카드만 등장 선언할 수 있습니다.", "CARD_TYPE_INVALID")[0],
    );
  }

  if (player.field[slot].card !== null) {
    violations.push(fail("빈 필드에만 등장할 수 있습니다.", "FIELD_OCCUPIED")[0]);
  }

  const sameNameExists = FIELD_SLOTS.some((fieldSlot) => {
    const fieldCard = player.field[fieldSlot].card;
    return (
      fieldCard?.sameNameKey &&
      card.sameNameKey &&
      fieldCard.sameNameKey === card.sameNameKey
    );
  });
  ...
  if (!inHand) {
    violations.push(
      fail("손패에 있는 카드만 등장 선언할 수 있습니다.", "CARD_NOT_IN_HAND")[0],
    );
  }
...
export function validateItemEquip(
  ...
  if (card.cardType !== "item") {
    violations.push(
      fail("아이템 카드만 장비 선언할 수 있습니다.", "CARD_TYPE_INVALID")[0],
    );
  }

  if (!field.card) {
    violations.push(
      fail("캐릭터가 있는 필드에만 아이템을 장비할 수 있습니다.", "NO_CHARACTER")[0],
    );
  }

  if (field.attachedItem) {
    violations.push(
      fail("아이템은 캐릭터 1장당 1장만 장비할 수 있습니다.", "ITEM_LIMIT")[0],
    );
  }
...
export function validateAreaPlace(
  ...
  if (card.cardType !== "area") {
    violations.push(
      fail("에리어 카드만 배치 선언할 수 있습니다.", "CARD_TYPE_INVALID")[0],
    );
  }

  if (field.area) {
    violations.push(
      fail("해당 필드에는 이미 에리어가 있습니다.", "AREA_LIMIT")[0],
    );
  }
...
export function validateAttackDeclaration(
  state: GameState,
  playerId: PlayerID,
  attackerCardId: string,
): RuleViolation[] {
  ...
  if (state.turn.activePlayer !== playerId || state.turn.phase !== "main") {
    violations.push(
      fail("공격 선언은 자기 메인페이즈에만 가능합니다.", "TIMING_INVALID")[0],
    );
  }
  ...
  if (!slot) {
    violations.push(
      fail("공격 캐릭터를 장에서 찾을 수 없습니다.", "CARD_NOT_FOUND")[0],
    );
    return violations;
  }
  ...
  if (!isAF(slot)) {
    violations.push(
      fail("공격은 AF 캐릭터만 할 수 있습니다.", "ATTACKER_NOT_AF")[0],
    );
  }

  const controller = findCardController(state, attackerCardId);
  if (controller !== playerId) {
    violations.push(
      fail("자신의 캐릭터로만 공격 선언할 수 있습니다.", "NOT_OWN_CARD")[0],
    );
  }
  ...
  if (card.isTapped) {
    violations.push(
      fail("행동 완료 상태의 캐릭터는 공격할 수 없습니다.", "ATTACKER_TAPPED")[0],
    );
  }

  if (card.canAttack === false) {
    violations.push(
      fail("이 캐릭터는 현재 공격할 수 없습니다.", "ATTACK_FORBIDDEN")[0],
    );
  }
...
export function validateDefenderSelection(
  ...
  if (!state.battle.isActive || !state.battle.attackerCardId) {
    violations.push(fail("현재 배틀 중이 아닙니다.", "NO_BATTLE")[0]);
    return violations;
  }
  ...
  if (!attackerSlot || !defenderSlot) {
    violations.push(
      fail("공격자 또는 방어자를 찾을 수 없습니다.", "CARD_NOT_FOUND")[0],
    );
    return violations;
  }

  if (!isDF(defenderSlot)) {
    violations.push(
      fail("방어 캐릭터는 DF 캐릭터여야 합니다.", "DEFENDER_NOT_DF")[0],
    );
  }

  if (getColumn(attackerSlot) !== getColumn(defenderSlot)) {
    violations.push(
      fail("방어 캐릭터는 같은 열이어야 합니다.", "NOT_SAME_COLUMN")[0],
    );
  }

  const owner = findCardController(state, defenderCardId);
  if (owner !== playerId) {
    violations.push(
      fail("자신의 캐릭터만 방어 캐릭터로 지정할 수 있습니다.", "NOT_OWN_CARD")[0],
    );
  }

  const defenderCell = state.players[playerId].field[defenderSlot];
  const defender = defenderCell.card;
  if (!defender) {
    violations.push(
      fail("방어 캐릭터를 장에서 찾을 수 없습니다.", "CARD_NOT_FOUND")[0],
    );
    return violations;
  }

  if (defender.isTapped) {
    violations.push(
      fail("행동 완료 상태의 캐릭터는 방어할 수 없습니다.", "DEFENDER_TAPPED")[0],
    );
  }

  if (defender.canBlock === false) {
    violations.push(
      fail("이 캐릭터는 현재 방어할 수 없습니다.", "BLOCK_FORBIDDEN")[0],
    );
  }
...
export function validateStepLikeMove(
  state: GameState,
  playerId: PlayerID,
  sourceCardId: string,
  toSlot: FieldSlot,
  allowedDestinations: FieldSlot[],
): RuleViolation[] {
  const violations: RuleViolation[] = [];
  ...
  if (state.battle.isActive) {
    violations.push(
      fail("배틀 중에는 이동 기본능력을 사용할 수 없습니다.", "BATTLE_FORBIDDEN")[0],
    );
  }

  if (!sourceSlot) {
    violations.push(fail("이동할 캐릭터가 장에 없습니다.", "CARD_NOT_FOUND")[0]);
    return violations;
  }

  if (!allowedDestinations.includes(toSlot)) {
    violations.push(
      fail("해당 기본능력으로 이동할 수 없는 위치입니다.", "DESTINATION_INVALID")[0],
    );
  }

  if (state.players[playerId].field[toSlot].card !== null) {
    violations.push(
      fail("빈 아군 필드로만 이동할 수 있습니다.", "FIELD_OCCUPIED")[0],
    );
  }
  ...
  if (card?.isTapped) {
    violations.push(
      fail("행동 완료 상태의 캐릭터는 이동할 수 없습니다.", "SOURCE_TAPPED")[0],
    );
  }
...
export function pushEvent(state: GameState, event: GameEvent): void {
  ...
  if (event.type === "DECK_BECAME_ZERO" && event.playerId) {
    state.winner = getOpponent(event.playerId);
  }

  if (event.type === "CONCEDE" && event.playerId) {
    state.winner = getOpponent(event.playerId);
  }
}
  ...
  for (const playerId of ["P1", "P2"] as const) {
    ...
    for (const slot of FIELD_SLOTS) {
      ...
      if (!cell.card && cell.attachedItem) {
        ...
        cell.attachedItem = null;
        state.logs.push(
          `[SBA] ${playerId}의 ${slot}에서 장착 대상이 사라져 아이템을 파기했습니다.`,
        );
      }
    }
    ...
    if (player.deck.length === 0 && !state.winner) {
      ...
      state.logs.push(
        `[WIN] ${playerId}의 덱이 0장이 되어 ${getOpponent(playerId)} 승리`,
      );
    }
  }
}
...
export function canResolvePendingTriggers(state: GameState): boolean {
  return (
    state.declarationStack.every((d) => d.resolved) &&
...
export function createDeclaredTargets(params: {
  ...
  if (params.targetCardIds) {
    for (const cardId of params.targetCardIds) {
      targets.push({
        kind: "card",
        cardId,
      });
  ...
  if (params.targetSlots) {
    for (const slot of params.targetSlots) {
      targets.push({
        kind: "field",
        slot,
      });
...
export function resolveDeclarationCore(
  state: GameState,
  declaration: Declaration,
): void {
  if (declaration.kind === "useCharacter") {
    const slot = declaration.declaredTargets.find((t) => t.kind === "field")?.slot;
    if (!slot || !declaration.sourceCardId) {
      state.logs.push(`[RESOLVE] useCharacter 해결 실패: 대상 슬롯 없음`);
    ...
    if (!card) {
      state.logs.push(`[RESOLVE] useCharacter 해결 실패: 카드 없음`);
      declaration.resolved = true;
      return;
    }
    ...
    state.logs.push(
      `[RESOLVE] ${declaration.playerId} 캐릭터 등장 해결: ${card.name} -> ${slot}`,
    );
    pushEvent(state, {
      type: "CHARACTER_ENTERED_FIELD",
  ...
  if (declaration.kind === "attack") {
    ...
    declaration.resolved = true;
    state.logs.push(
      `[RESOLVE] ${declaration.playerId} 공격 선언 해결: ${declaration.sourceCardId}`,
    );
    pushEvent(state, {
      type: "ATTACK_DECLARATION_RESOLVED",
      playerId: declaration.playerId,
...
export function resolveBattleCore(state: GameState): void {
  ...
  if (!attackerPlayerId || !defenderPlayerId || !attackerCardId) {
